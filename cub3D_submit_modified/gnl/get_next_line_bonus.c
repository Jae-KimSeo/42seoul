...
/*   Updated: 2021/01/19 23:17:54 by toispre          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int						get_next_line(int fd, char **line)
{
	...
	if (fd < 0 || fd >= OPEN_MAX || BUFFER_SIZE <= 0 || !line)
		return (-1);
	if ((i = buffer_store(&backup[fd], fd)) == -1)
		return (-1);
	if (i == BUFFER_SIZE || ft_strchr(backup[fd], '\n'))
	{
		split_ptr = ft_split_line(backup[fd]);
		*line = split_ptr[0];
		tmp = backup[fd];
		backup[fd] = split_ptr[1];
		free(tmp);
		free(split_ptr);
	}
	else
