...
/*   Updated: 2021/02/17 12:31:04 by jae-kim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		float floor_y, int loop[2])
{
	...
	t[1] = (int)(TEX_HEIGHT * (floor_y - cell_y)) & (TEX_HEIGHT - 1);
...
void				floor_casting(t_info *info)
{
	...
	while (loop[1] < g_height)
	{
		...
		while (loop[0] < g_width)
		{
			...
			floor_x += row_distance * (ray.ray_dir_x1 - ray.ray_dir_x0) / g_width;
			floor_y += row_distance * (ray.ray_dir_y1 - ray.ray_dir_y0) / g_width;
			loop[0]++;
		}
		++loop[1];
	}
